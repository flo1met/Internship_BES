...
data_error <- left_join(data_comp_filt, data_part_filt) %>%
  ...
  mutate(
    ...
    MSD = PMP1u - BES_PMP1u # mean signed difference
  )
...
ggsave("../plots/boxplot_MAPE.pdf", width = 7, height = 7)

data_error %>% filter(d == 0.2, c == 0.2) %>%
  ggplot(aes(x = as.factor(n_sample), y = MSD)) +
  stat_summary(aes(x = as.factor(n_sample), y = MSD), 
               fun = mean, 
               geom = "line", 
               group = 1, 
               linewidth = 0.2) +
...
ggsave("../plots/boxplot_MSD.pdf", width = 7, height = 7)
...
ggsave("../plots/lineplot_MAPE.pdf", width = 7, height = 7)

agg_errors %>% filter(d != 0) %>% ggplot() +
  geom_line(aes(x = as.factor(n_sample), y = MSD, color = as.factor(d), group = 1)) +
  labs(x = "Effect Size", y = "Correlation") +
  scale_color_discrete(guide = "none") +
  theme_bw() +
  facet_grid(c~ d) +
...
ggsave("../plots/lineplot_MSD.pdf", width = 7, height = 7)
