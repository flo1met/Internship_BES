...
# UI
ui <- navbarPage(
  title = "Simulation Study Explorer",
  ...
  tabPanel("Simulation 1",
           sidebarLayout(
             sidebarPanel(
               ...
               conditionalPanel(
                 condition = "input.sim1_tabs == 'Plot'",
                 h4("Data Filters"),
               ...
               conditionalPanel(
                 condition = "input.sim1_tabs == 'Summary Statistics'",
                 h4("Summary Filters"),
               ...
               conditionalPanel(
                 condition = "input.sim1_tabs == 'Data Table'",
                 h4("Table Filters"),
             ...
             mainPanel(
               ...
               tabsetPanel(
                 ...
                 tabPanel("Help",
                          h3("Simulation 1 Guide"),
                          p("This simulation investigates MAPD and MSD metrics for correctly and incorrectly specified hypotheses."),
                          tags$ul(
  ...
  tabPanel("Simulation 2",
           sidebarLayout(
             sidebarPanel(
               ...
               conditionalPanel(
                 condition = "input.sim2_tabs == 'Line Plots' || input.sim2_tabs == 'Data Table'",
                 h4("Data Filters"),
                 
                 checkboxGroupInput("sim2_d", "Effect Size (d):",
                                    choices = list("d = 0.2" = "0.2",
                                                   "d = 0.5" = "0.5",
                                                   "d = 0.8" = "0.8"),
                                    selected = c("0.2", "0.5", "0.8")),
                 
                 checkboxGroupInput("sim2_c", "Correlation (c):",
                                    choices = list("c = 0.0" = "0.0", 
                                                   "c = 0.2" = "0.2", 
                                                   "c = 0.5" = "0.5",
                                                   "c = 0.8" = "0.8"),
                 ...
                 checkboxGroupInput("sim2_n", "Sample Size:",
                                    choices = NULL,
                                    selected = NULL)
               ),
               
               conditionalPanel(
                 condition = "input.sim2_tabs == 'Distribution Plots'",
                 h4("Distribution Plot Settings"),
                 
                 selectInput("sim2_d_single", "Effect Size (d):",
                             choices = NULL,
                             selected = NULL),
                 
                 selectInput("sim2_c_single", "Correlation (c):",
                             choices = NULL,
                             selected = NULL),
                 
                 selectInput("sim2_plot_type", "Plot Type:",
                             choices = c(
                               "PMP vs BES - Complement" = "comp_c",
                               "PMP vs BES - Unconstrained" = "comp_u"
                             ),
                             selected = "comp_c")
               )
             ),
             ...
             mainPanel(
               ...
               tabsetPanel(
                 ...
                 tabPanel("Line Plots",
                          h4("Comparison of PMPs: BES vs Complete Hypothesis"),
                          plotOutput("sim2_lineplot_c", height = "500px"),
                          br(),
                          plotOutput("sim2_lineplot_u", height = "500px")
                 ),
                 
                 tabPanel("Distribution Plots",
                          h4("Distribution Comparison"),
                          plotOutput("sim2_dist_plot", height = "600px")
                 ),
                 
                 tabPanel("Data Table",
                          h4("Simulation 2 Results"),
                          DTOutput("sim2_table")
                 ),
                 
                 tabPanel("Help",
                          h3("Simulation 2 Guide"),
                          p("This simulation compares Bayesian Evidence Synthesis (BES) with complete hypothesis testing."),
                          tags$ul(
                            tags$li(strong("Line Plots:"), "Compare PMP trends for BES (partial hypotheses) vs complete hypothesis across sample sizes."),
                            tags$li(strong("Distribution Plots:"), "View distributions of PMPs and BES values for specific conditions."),
                            tags$li(strong("Complement:"), "Tests against the complement hypothesis (all orderings not in the tested hypothesis)."),
                            tags$li(strong("Unconstrained:"), "Tests against the unconstrained hypothesis (no ordering restrictions).")
                          )
                 )
               )
             )
  ...
  tabPanel("About",
           fluidRow(
             column(10, offset = 1,
                    h2("Bayesian Evidence Synthesis: Simulation Studies"),
                    br(),
                    h4("Overview"),
                    p("This application presents results from two simulation studies investigating Bayesian Evidence Synthesis (BES) for aggregating evidence from partially overlapping hypotheses."),
                    br(),
                    h4("Simulation 1: Correctly and Incorrectly Specified Hypotheses"),
                    p("Examines the accuracy of BES when aggregating partial hypotheses using MAPD and MSD metrics."),
                    br(),
                    h4("Simulation 2: Real-world Application Scenario"),
                    p("Compares BES performance against complete hypothesis testing in practical scenarios."),
                    br(),
                    tags$hr(),
                    p(strong("Reference:"), "Metwaly, F. et al. Bayesian Evidence Synthesis: Aggregating Evidence from Partially Overlapping Hypotheses")
             )
           )
  )
)
...
server <- function(input, output, session) {
  ...
  observe({
    ...
    updateCheckboxGroupInput(session, "sim2_n",
                             choices = setNames(as.character(unique_n), 
                                                paste("n =", unique_n)),
                             selected = as.character(unique_n))
    
    updateSelectInput(session, "sim2_d_single",
                      choices = setNames(as.character(unique_d), 
                                         paste("d =", unique_d)),
                      selected = "0.2")
    
    updateSelectInput(session, "sim2_c_single",
                      choices = setNames(as.character(unique_c), 
                                         paste("c =", unique_c)),
                      selected = "0.2")
  })
  ...
  # ============ SIM 2 FILTERED DATA ============
  filtered_sim2_data <- reactive({
    req(input$sim2_d, input$sim2_c, input$sim2_n)
    
    agg_data_sim2() %>%
      filter(
        d %in% as.numeric(input$sim2_d),
        c %in% as.numeric(input$sim2_c),
        n_sample %in% as.numeric(input$sim2_n),
        d != 0
      )
  })
  ...
  # ============ SIM 2 PLOTS ============
  output$sim2_lineplot_c <- renderPlot({
    data <- filtered_sim2_data()
  ...
  output$sim2_lineplot_u <- renderPlot({
    data <- filtered_sim2_data()
  ...
  output$sim2_dist_plot <- renderPlot({
    req(input$sim2_d_single, input$sim2_c_single, input$sim2_plot_type)
    
    data <- load_data_sim2() %>%
      filter(
        n_sample %in% c(75, 150, 300, 1500),
        c == as.numeric(input$sim2_c_single),
        d == as.numeric(input$sim2_d_single)
      )
    
    if (input$sim2_plot_type == "comp_c") {
      # Complement comparison
      x_min <- min(data$PMP1c_H, data$BESc, na.rm = TRUE)
      x_max <- max(data$PMP1c_H, data$BESc, na.rm = TRUE)
      
      p1 <- ggplot(data, aes(y = factor(n_sample), x = PMP1c_H, fill = factor(ind_p_all))) +
        geom_boxplot(width = 0.12, outlier.color = NA, alpha = 0.5) +
        stat_dots(side = "right", justification = 1.2, alpha = 0.2) +
      ...
      p2 <- ggplot(data, aes(y = factor(n_sample), x = BESc, fill = factor(ind_comp))) +
        geom_boxplot(width = 0.12, outlier.color = NA, alpha = 0.5) +
        stat_dots(side = "right", justification = 1.2, alpha = 0.2) +
        scale_x_continuous(limits = c(x_min, x_max)) +
        scale_fill_tq() +
      ...
      x_min <- min(data$PMP1u_H, data$BESu, na.rm = TRUE)
      x_max <- max(data$PMP1u_H, data$BESu, na.rm = TRUE)
      
      p1 <- ggplot(data, aes(y = factor(n_sample), x = PMP1u_H, fill = factor(ind_p_all))) +
        geom_boxplot(width = 0.12, outlier.color = NA, alpha = 0.5) +
        stat_dots(side = "right", justification = 1.2, alpha = 0.2) +
        scale_x_continuous(limits = c(x_min, x_max)) +
        scale_fill_tq() +
      ...
      p2 <- ggplot(data, aes(y = factor(n_sample), x = BESu, fill = factor(ind_comp))) +
        geom_boxplot(width = 0.12, outlier.color = NA, alpha = 0.5) +
        stat_dots(side = "right", justification = 1.2, alpha = 0.2) +
        scale_x_continuous(limits = c(x_min, x_max)) +
        scale_fill_tq() +
  ...
  output$sim2_table <- renderDT({
    ...
    data %>%
      ...
      datatable(options = list(pageLength = 25, scrollX = TRUE),
                rownames = FALSE) %>%
      formatRound(columns = c("PMP1u_H", "PMP1c_H", "BESc", "BESu"), digits = 4)
  })
}
